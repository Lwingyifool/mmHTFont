...
class HTFFactory {
    ...
    protected static int[] getStyleForTextView(TextView view) {
        ...
        if (styleIds[0] == -1) {
            ...
            styleIds[0] = HTFConfig.get().getClassStyles().containsKey(view.getClass())
                    ? HTFConfig.get().getClassStyles().get(view.getClass())
    ...
    protected static boolean parentIsToolbarV7(View view) {
        return HTFUtils.canCheckForV7Toolbar() && view.getParent() != null && (view.getParent() instanceof Toolbar);
    }
    ...
    public HTFFactory(int attributeId) {
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs) {
        if (view instanceof TextView) {
            ...
            if (HTFTypefaceUtils.isLoaded(((TextView) view).getTypeface())) {
                return;
            }
            ...
            if (TextUtils.isEmpty(textViewFont)) {
                ...
                if (styleForTextView[1] != -1)
                    textViewFont = HTFUtils.pullFontPathFromTheme(context, styleForTextView[0], styleForTextView[1], mAttributeId);
                else
                    textViewFont = HTFUtils.pullFontPathFromTheme(context, styleForTextView[0], mAttributeId);
            }
            ...
            HTFUtils.applyFontToTextView(context, (TextView) view, HTFConfig.get(), textViewFont, deferred);
        }

        // AppCompat API21+ The ActionBar doesn't inflate default Title/SubTitle, we need to scan the
        // Toolbar(Which underlies the ActionBar) for its children.
        if (HTFUtils.canCheckForV7Toolbar() && view instanceof Toolbar) {
        ...
        if (view instanceof HasTypeface) {
        ...
        } else if (HTFConfig.get().isCustomViewTypefaceSupport() && HTFConfig.get().isCustomViewHasTypeface(view)) {
            final Method setTypeface = HTFReflectionUtils.getMethod(view.getClass(), "setTypeface");
            ...
            if (setTypeface != null && typeface != null) {
                HTFReflectionUtils.invokeMethod(view, setTypeface, typeface);
            }
        }
    ...
    private Typeface getDefaultTypeface(Context context, String fontPath) {
        if (TextUtils.isEmpty(fontPath)) {
            fontPath = HTFConfig.get().getFontPath();
        }
        if (!TextUtils.isEmpty(fontPath)) {
            return HTFTypefaceUtils.load(context.getAssets(), fontPath);
        }
        return null;
    }
    ...
    private String resolveFontPath(Context context, AttributeSet attrs) {
        ...
        String textViewFont = HTFUtils.pullFontPathFromView(context, attrs, mAttributeId);

        // Try view style attributes
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = HTFUtils.pullFontPathFromStyle(context, attrs, mAttributeId);
        }

        // Try View TextAppearance
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = HTFUtils.pullFontPathFromTextAppearance(context, attrs, mAttributeId);
        }
