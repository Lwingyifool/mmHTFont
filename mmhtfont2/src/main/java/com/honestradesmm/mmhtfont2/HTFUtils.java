...
public final class HTFUtils {
    ...
    public static CharSequence applyTypefaceSpan(CharSequence s, Typeface typeface) {
        if (s != null && s.length() > 0) {
            ...
            ((Spannable) s).setSpan(HTFTypefaceUtils.getSpan(typeface), 0, s.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return s;
    }
    ...
    static boolean applyFontToTextView(final Context context, final TextView textView, final String filePath, boolean deferred) {
        ...
        final Typeface typeface = HTFTypefaceUtils.load(assetManager, filePath);
        return applyFontToTextView(textView, typeface, deferred);
    }

    static void applyFontToTextView(final Context context, final TextView textView, final HTFConfig config) {
        applyFontToTextView(context, textView, config, false);
    }

    static void applyFontToTextView(final Context context, final TextView textView, final HTFConfig config, boolean deferred) {
     ...
     *                     defined font.
     */
    public static void applyFontToTextView(final Context context, final TextView textView, final HTFConfig config, final String textViewFont) {
        applyFontToTextView(context, textView, config, textViewFont, false);
    }

    static void applyFontToTextView(final Context context, final TextView textView, final HTFConfig config, final String textViewFont, boolean deferred) {
        if (context == null || textView == null || config == null) return;
